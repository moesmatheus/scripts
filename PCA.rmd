...
# as correlações entre as vars e os fatores sao dados por correlação (Zi, CPj) = wij * desvio padrao (CP) chamadas components loadings ou 'cargas das componentes" Vamos considerar apenas as 3 primeiras componentes
...
```{r}
biplot(aval.cp, xlab="cp1",ylab="cp2",xlabs=AVALIACOES$PROFESSOR)
abline(h=0,v=0,col="darkblue",lty=5)
```

```{r}
plot(final$PC1,final$PC2,type = "n")
text(final$PC1,final$PC2,AVALIACOES$PROFESSOR)
